...
bool CMCIObject::GetTrackOrMediaLength(BYTE track, BYTE& min, BYTE& sek, BYTE& frame) {
	...
	MCI_STATUS_PARMS status; // checks time format of different media
	status.dwItem = MCI_STATUS_TIME_FORMAT;
	...
	case MCI_FORMAT_TMSF:
		...
		if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_STATUS,
			MCI_STATUS_ITEM | MCI_TRACK, (DWORD_PTR)(LPMCI_STATUS_PARMS)&length)) != 0) {
			MCIError();
			return false;
		}
	...
	case MCI_FORMAT_MILLISECONDS:
		...
		if ((m_Result = mciSendCommand(m_op.wDeviceID, MCI_STATUS, MCI_STATUS_ITEM, (DWORD_PTR)(LPMCI_STATUS_PARMS)&length)) != 0) {
			MCIError();
			return false;
		}
